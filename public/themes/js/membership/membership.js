$(document).ready(() => {
    ...
    $("#plan-1").click(() => {
        window.location.href = "/membership/checkout?plan=1"
    });
    $("#plan-2").click(() => {
        window.location.href = "/membership/checkout?plan=2"
    });
    $("#plan-3").click(() => {
        window.location.href = "/membership/checkout?plan=3"
    });
    ...
    $("#apply-promo-code").click(() => {
        $.ajax({
            ...
            success: function (data) {
                var total_price_product = document.getElementsByClassName("amount-membership");
                if (data.status == "1") {
                    $(".set-div-promo").html("");
                    ...
                    $(".set-div-promo").show();
                    $(".amount-membership").text("$" + (Number(trimSpace(total_price_product[0].textContent).split("$")[1]) - Math.round(data.message * Number(trimSpace(total_price_product[0].textContent).split("$")[1]) / 100)));
