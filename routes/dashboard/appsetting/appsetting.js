...
>>>>>>> c8ee27608d1239388a3aa4ea279299a50d2ddaae
                });
            } else {
...
router.post("/appsettings", checkAdmin, (req, res) => {
    try {
        ...
        appsettingModels.findOne({ idApp: req.body.appid }).then((dataOne) => {
                if (dataOne) {
                    ...
                    dataOne.status = true;
                    dataOne.save().then(() => {
                            return res.json({ status: "1", content: 'Saved' });
                        })
                        // dataOne.oneSignalID = req.body.onesignalid;

                } else {
                    inforappModels.findOne({ idApp: req.body.appid }).then((dataCheck) => {
                        if (dataCheck) {
                            ...
                            return res.json({ status: "3", content: 'Not Find Application Identifier' });
                        }
                    });
                }
            })
